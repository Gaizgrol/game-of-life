 ...
 * @param {number} max 
 */
const valorNoIntervalo = ( valor, min, max ) =>
{
...
class Grade
{
    ...
    atualiza()
    {
        ...
        for ( let y=0; y<h; y++ )
        {
            for ( let x=0; x<w; x++ )
            {
                ...
                const ativos = this.vizinhos( x, y ).reduce( ( ativ, i ) => ativ + Boolean( grade.celulas[i] ), 0 );
    ...
    indice( x, y, verificaLimites = true )
    {
        ...
        return (
            valorNoIntervalo( x, 0, this.largura-1 ) &&
